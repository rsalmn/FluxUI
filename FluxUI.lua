...
local function GetConfigList()
    -- Check apakah file functions tersedia dan folder ada
    local folderOk, folderExists = pcall(isfolder, ConfigFolder)
...
function FluxUI:CreateWindow(config)
    ...
    local function startBubblePulse()
        ...
        -- Pakai RegisterUpdate agar consistent dengan sistem dan bisa di-cleanup via FluxUI:Destroy()
        pulseConnection = RegisterUpdate(function()
            if not isMinimized then
                UnregisterUpdate(pulseConnection)
                pulseConnection = nil
                return
            end
            if not BubbleFrame or not BubbleFrame.Parent then
                UnregisterUpdate(pulseConnection)
                pulseConnection = nil
                return
            end
        ...
        end, 1) -- interval 1 = every heartbeat frame
    end
    ...
    function Window:CreateTab(tabName, icon, lazyLoad)
        ...
        function Tab:CreateToggle(config)
            ...
            -- NOTE: RegisterThemedElement untuk color dipindahkan ke setelah toggleObj dan ToggleCircle dibuat
            ...
            -- Register themed colors SETELAH ToggleCircle dan toggleObj dibuat
            RegisterThemedElement(function(colors)
                Tween(ToggleButton, {BackgroundColor3 = toggled and colors.Accent or colors.Border}, 0.2)
        ...
        function Tab:CreateSlider(config)
            ...
            -- NOTE: RegisterThemedElement untuk Tooltip dan TooltipText dipindahkan ke setelah keduanya dibuat
            ...
            -- Register themed colors SETELAH Tooltip dan TooltipText dibuat
            ...
            local inputChangedConn = UserInputService.InputChanged:Connect(function(input)
                if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
                    if not SliderBar or not SliderBar.Parent then
                        dragging = false
                        return
                    end
            ...
            local sliderObj = {
                ...
                Destroy = function()
                    -- Disconnect tracked global connections
                    if inputEndedConn then inputEndedConn:Disconnect() inputEndedConn = nil end
                    if inputChangedConn then inputChangedConn:Disconnect() inputChangedConn = nil end
                    if SliderFrame and SliderFrame.Parent then SliderFrame:Destroy() end
                end,
        ...
        function Tab:CreateColorPicker(config)
            ...
            local cpInputConn = UserInputService.InputChanged:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseMovement then
                    if not ColorFrame or not ColorFrame.Parent then
                        draggingSV = false
                        draggingHue = false
                        return
                    end
            ...
            return {
                ...
                Destroy = function()
                    -- Disconnect tracked global connection
                    if cpInputConn then cpInputConn:Disconnect() cpInputConn = nil end
                    if ColorFrame and ColorFrame.Parent then ColorFrame:Destroy() end
                end,
                Instance = ColorFrame
            }
        end
